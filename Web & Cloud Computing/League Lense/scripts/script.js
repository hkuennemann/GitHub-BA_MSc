...
function addLeagueSelection(league) {
    ...
    if (!isDuplicate) {
        ...
        // Notify the user
        Toastify({
            text: `${league.league.name} was added to favorites`,
            duration: 2000,
            close: true,
            gravity: "top",
            position: "right",
        ...
        // Notify the user
        Toastify({
            text: `${league.league.name} is already in favorites`,
            duration: 2000,
            close: true,
            gravity: "top",
            position: "right",
...
function displayAllLeagues(data) {
    ...
    data.response.forEach((resp) => {
        // Check if the league type is "League" before proceeding
        if (resp.league.type === "League") {
            const row = tableBody.insertRow();
            ...
            goLink.style.color = 'white';
            goLink.onclick = () => {
                localStorage.setItem('selectedLeagueId', resp.league.id);
                localStorage.setItem('selectedLeagueName', resp.league.name);
                localStorage.setItem('selectedLeagueLogo', resp.league.logo);
                localStorage.setItem('selectedCountryName', resp.country.name);
                window.location.href = 'standings.html';
            };
            goCell.appendChild(goLink);
        }
...
// Function to display the fixtures in a table
...
function showData_lineUps(data) {
    ...
    generateFormationGrid('team1-formation', team1.startXI);
    ...
    generateFormationGrid('team2-formation', team2.startXI);
...
function generateFormationGrid(formationDivId, startXI) {
